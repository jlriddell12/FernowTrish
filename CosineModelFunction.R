...
cosinemodel <- function(input_folder, output_folder) {
  ...
  for (file in files) {
    ...
    # Assuming the columns are "DateTime" and "Temperature"
    colnames(d) <- c('date', 'temp')
    ...
    # Calculate decimal dates relative to the reference date
    d$decimalDate <- (d$dateNumeric - ref) / 365
    ...
    # Add the model's fitted values to the data frame
    d$model <- temp2$fitted.values
